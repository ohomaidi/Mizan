...
| 11a — SharePoint tenant external-sharing | 4 baselines on `/admin/sharepoint/settings`: strict external sharing, default link Internal+View, domain allow-list (placeholder), anonymous off. Singleton model — push only, no rollback (audit captures diff). | ✅ Shipped |
| 14 — Defender for Endpoint ASR rules | 6 baselines via Intune Endpoint Protection profile: Office child processes, executable email content, LSASS credential theft, JS/VBS launch exe, PSExec+WMI, untrusted USB. Audit mode default. | ✅ Shipped |
| 14b — IOC push (Defender for Endpoint) | Console for file hash / URL / domain / IPv4 / IPv6 indicator authoring. `POST /security/tiIndicators` create + `DELETE` rollback. Idempotency by description tag. | ✅ Shipped |
