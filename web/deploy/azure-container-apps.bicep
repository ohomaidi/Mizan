...
@description('Override the Container App name (2-32 chars, lowercase alphanumerics + single hyphens, must start with a letter and end alphanumeric — the value is lowercased automatically, and prefixed with "ca" if it starts with a non-letter). Default: <namePrefix>-app-<suffix>.')
@maxLength(32)
param containerAppName string = ''

@description('Override the Key Vault name (3-24 chars, alphanumeric + hyphens, must start with a letter, globally unique — prefixed with "kv" automatically if it starts with a non-letter). Default: <namePrefix>-kv-<suffix> truncated to 24.')
@maxLength(24)
...
@description('Override the storage account name (3-24 chars, lowercase alphanumeric only — hyphens are stripped and the value is lowercased automatically). Default: <namePrefix><suffix> with hyphens stripped, truncated to 24.')
@maxLength(24)
...
// 24-char limit too.
//
// v2.8.2: the Container App and storage account names are normalized
// (lowercased, hyphens stripped for storage) on BOTH the override and
// fallback paths. ARM parameters cannot carry regex constraints, so the
// portal form happily accepts a camelCase Container App name and the
// deployment then dies with "Invalid ContainerApp name" — normalizing
// here turns the overwhelmingly common mistake (uppercase) into a
// working deployment instead. @maxLength on the override params gives
// the portal form the length limits it CAN enforce client-side.
...
var actualStorageName = take(toLower(replace(empty(storageAccountName) ? '${namePrefix}${uniq}' : storageAccountName, '-', '')), 24)
// v2.8.3: Key Vault and Container App names must START WITH A LETTER —
// a date-style override like "28jul…" fails at submit with
// VaultNameNotValid. When the first character isn't a letter, prefix
// the name ("kv" / "ca") and re-truncate to the type's limit.
var letters = 'abcdefghijklmnopqrstuvwxyz'
var kvNameCandidate = empty(keyVaultName) ? take('${namePrefix}-kv-${uniq}', 24) : keyVaultName
var actualKvName = contains(letters, toLower(first(kvNameCandidate))) ? kvNameCandidate : take('kv${kvNameCandidate}', 24)
...
var appNameCandidate = toLower(empty(containerAppName) ? '${namePrefix}-app-${uniq}' : containerAppName)
var actualAppName = contains(letters, first(appNameCandidate)) ? appNameCandidate : take('ca${appNameCandidate}', 32)
...
resource storage 'Microsoft.Storage/storageAccounts@2024-01-01' = {
  // BCP334 is a theoretical-length warning: the analyzer sees the
  // normalization chain could produce < 3 chars, but that requires an
  // empty namePrefix AND an empty/hyphens-only override — not a real
  // configuration.
...
resource app 'Microsoft.App/containerApps@2024-10-02-preview' = {
  ...
  // dependsOn forces Bicep to finish every secret and the role
  // assignment module before the Container App is created.
  dependsOn: [
...
// v2.8.1: both role assignments (Container Apps Contributor on the RG,
// Key Vault Secrets Officer on the vault) moved into a module so their
// names can be seeded on the UAMI's *principalId* instead of its
// resource path — see role-assignments.bicep for the full rationale.
// Short version: a uami.id-seeded name survives a UAMI delete +
// recreate with the SAME name but a DIFFERENT principal, and Azure
// refuses that update (RoleAssignmentUpdateNotPermitted). The portal's
// resource list hides role assignments, so a "select all → delete"
// cleanup leaves them behind as orphans and bricks every redeploy into
// that resource group. principalId seeding makes a recreated identity
// mint fresh assignment names; orphans linger harmlessly.
//
// One-time migration for deployments created with v2.7.16 – v2.8.0:
// the old uami.id-seeded assignments still exist bound to the SAME
// live principal, so the first redeploy with this template fails with
// RoleAssignmentExists. Delete the two old Mizan UAMI assignments
// first, then redeploy:
//   az role assignment list --resource-group <rg> -o table
//   az role assignment list --scope "$(az keyvault show -n <vault> -g <rg> --query id -o tsv)" -o table
//   az role assignment delete --ids <each-mizan-uami-assignment-id>
module roleAssignments 'role-assignments.bicep' = {
  name: 'mizan-role-assignments'
  params: {
    principalId: uami.properties.principalId
    keyVaultName: kv.name
    containerAppsContributorRoleId: containerAppsContributorRoleId
    kvSecretsOfficerRoleId: kvSecretsOfficerRoleId
  }
}
