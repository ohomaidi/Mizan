// Mizan — Azure Container Apps deployment, NFS variant.
//
// Use this Bicep when the host subscription's policy blocks
// `allowSharedKeyAccess: true` on storage accounts (common in MCA-managed
// subscriptions). NFS 4.1 doesn't use shared-key auth at all — access is
// controlled by network rules, which we lock down to a private endpoint.
//
